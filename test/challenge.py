...
class UnderTheHoodGame:
    def start_game(self):
        self.run()  # Call the run method to start the game
        return
    def __init__(self):
        ...
        try:
            engine_bay_path = os.path.join(self.base_dir, "engine_bay.png")
            self.engine_background = pygame.image.load(engine_bay_path)
            self.engine_background = pygame.transform.scale(self.engine_background, (1200, 650))
        except:
            ...
            print(f"Warning: {engine_bay_path} not found. Using default background.")

        # Load sound effects (optional)
        try:
            self.correct_sound = pygame.mixer.Sound(os.path.join(self.base_dir, "correct.wav"))
            self.wrong_sound = pygame.mixer.Sound(os.path.join(self.base_dir, "wrong.wav"))
            self.win_sound = pygame.mixer.Sound(os.path.join(self.base_dir, "win.wav"))
            self.lose_sound = pygame.mixer.Sound(os.path.join(self.base_dir, "lose.wav"))
        ...
        self.component_descriptions = {
            ...
            'B': "Where you add oil to the engine. Use manufacturer recommended oil type.",
            'C': "Measures oil level in the engine. Check when engine is cool and on level ground.",
            'D': "Stores brake fluid for the hydraulic braking system. Critical for safe braking.",
    ...
    def load_component_images(self):
        component_data = [
            ('A', "washer_reservoir.png", "Windshield Washer Reservoir", (100, 450), (130, 130)),
            ('B', "brake_fluid.png", "Oil Cap", (250, 360), (100, 80)),
            ('C', "oil_dipstick.png", "Oil Dipstick", (525, 350), (20, 60)),
            ('D', "oil_cap.png", "Brake Fluid Reservoir", (210, 250), (180, 120)),
            ('E', "coolant_reservoir.png", "Coolant Reservoir", (980, 525), (110, 60)),
            ('F', "battery.png", "Battery", (930, 310), (230, 230))
        ]
        ...
        for key, img_path, name, pos, size in component_data:
            ...
            try:
                ...
                img = pygame.image.load(img_full_path).convert_alpha()
                img = pygame.transform.scale(img, size)
            except:
                img = pygame.Surface(size, pygame.SRCALPHA)
                img.fill(colors[key])
                # Add placeholder details (as in your original code)
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            # Handle key presses
                ...
                # Exit the game loop by setting the game state to None
                    self.screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
                    global SCREEN_WIDTH, SCREEN_HEIGHT
                    SCREEN_WIDTH, SCREEN_HEIGHT = self.screen.get_size()
                    self.game_state = None
                    
                    return  # Exit the method to stop further event handling
                ...
                for component, data in self.components.items():
                    ...
                    if (comp_rect.collidepoint(adjusted_mouse_x, adjusted_mouse_y) and
                        ...
                        if component in self.component_descriptions:
                            tooltip_text = f" {self.component_descriptions[component]}"
                            self.tooltip = {
                                'text': tooltip_text,
                                'position': (
    ...
    def draw(self):
        ...
        if self.game_state == GAME_WON:
            ...
            restart_text = self.font.render("Press R to play again and ESC to exit", True, BLACK)
    ...
    def run(self):
        while self.game_state is not None:  # Continue running while the game state is valid
            self.handle_events()
            self.update()
            self.draw()
            self.clock.tick(FPS)
