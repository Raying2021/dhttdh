...
def start_minigame():
    ...
    try:
        ...
        oil_cap = pygame.image.load(os.path.join(base_dir, 'oil_cap.png'))
        oil_cap = pygame.transform.scale(oil_cap, (80, 80))

        oil_meter = pygame.image.load(os.path.join(base_dir, 'Oil_Meter.jpg'))
        oil_meter = pygame.transform.scale(oil_meter, (100, 100))

        inserted_meter = pygame.image.load(os.path.join(base_dir, 'Oil_meter_in.png'))
        inserted_meter = pygame.transform.scale(inserted_meter, (200, 200))

        unfilled_meter = pygame.image.load(os.path.join(base_dir, 'Oil_meter_unfilled.png'))
    ...
    # Clickable areas
    oil_cap_rect = oil_cap.get_rect(center=(SCREEN_WIDTH // 5,
                                            SCREEN_HEIGHT // 2 + 23))
    oil_meter_rect = oil_meter.get_rect(bottomright=(SCREEN_WIDTH - 20,
                                                     SCREEN_HEIGHT - 20))
    inserted_meter_rect = inserted_meter.get_rect(center=(SCREEN_WIDTH // 4.5,
                                                          SCREEN_HEIGHT // 2))
    unfilled_meter_rect = unfilled_meter.get_rect(center=(SCREEN_WIDTH // 2,
                                                          SCREEN_HEIGHT // 2))

    # Animation variables
    oil_cap_alpha = 255
    oil_cap_y = SCREEN_HEIGHT // 2 + 23
    oil_cap_rising = False

    # Scene 2 transition variables
    scene2_timer = 60  # 1 second at 60 fps
    fade_progress = 0  # For fade in/out effects
    fade_in = True
    fade_out = False
    scene2_complete = False
    ...
    def create_text_surface(text):
        ...
        padding = 10
        background = pygame.Surface((text_surface.get_width() + padding * 2,
    ...
    error_rect = error_text.get_rect(bottom=SCREEN_HEIGHT - 10,
        ...
        if current_scene == SCENE_1:
            ...
            if oil_cap_alpha > 0:
                ...
                if oil_cap_hover and not oil_cap_rising:
                    ...
                    pygame.draw.rect(highlight, (255, 255, 0, 150),
                                     highlight.get_rect(), 5)
            ...
            if not fade_in:
                if inserted_meter_rect.collidepoint(mouse_pos):
                    highlight = pygame.Surface(inserted_meter.get_size(),
                                               pygame.SRCALPHA)
                    pygame.draw.rect(highlight, (255, 255, 0, 150),
                                     highlight.get_rect(), 5)
            ...
            if not fade_in:
                screen.blit(
                    lack_oil_text,
                    lack_oil_text.get_rect(bottom=SCREEN_HEIGHT - 10,
                                           centerx=SCREEN_WIDTH // 2))
                if unfilled_meter_hover:
                    highlight = pygame.Surface(unfilled_meter.get_size(),
                                               pygame.SRCALPHA)
                    pygame.draw.rect(highlight, (255, 255, 0, 150),
                                     highlight.get_rect(), 5)
